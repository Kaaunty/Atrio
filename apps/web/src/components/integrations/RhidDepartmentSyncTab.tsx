...
interface RhidDepartmentSyncTabProps {
  onOpenSettings: () => void;
}

export const RhidDepartmentSyncTab: React.FC<RhidDepartmentSyncTabProps> = ({ onOpenSettings }) => {
  ...
  const loadOverview = async () => {
    try {
      ...
      const overview = await integrationService.getRhidDepartmentsOverview();
      setData(overview);
    } catch (err: any) {
      console.error('Falha ao carregar visão de departamentos:', err);
      setActionMessage({
        type: 'error',
  ...
  const handleImport = async (deptIds?: number[]) => {
    try {
      setSyncing(true);
      setActionMessage(null);

      let idsToImport = deptIds;
      ...
      const res = await integrationService.importRhidDepartments(idsToImport);
      setFeedback({
        isOpen: true,
        type: 'success',
      ...
      setFeedback({
        ...
        description: err.response?.data?.message || err.message || 'Erro ao importar departamentos do RHiD.',
      });
    } finally {
      setSyncing(false);
    }
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-700 font-bold uppercase tracking-wider text-[11px]">
                ...
                <th className="p-3.5">Nome do Departamento</th>
